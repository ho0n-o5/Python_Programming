...
# 총 재고 금액 출력
print(f"총액: {sum(i[1] * i[2] for i in stocks):,}원")

print(
    f"총액: {sum([price * qty for _, price, qty in stocks]):,}원"
)  # 리스트부터 만드는거라 메모리 소모가 큼
print(
    f"총액: {sum(price * qty for _, price, qty in stocks):,}원"
)  # 메모리상 이득(내가 찾은거)
...
print(f"총액: {sum(price * qty for _, price, qty in zip(*stocks)):,}원")
